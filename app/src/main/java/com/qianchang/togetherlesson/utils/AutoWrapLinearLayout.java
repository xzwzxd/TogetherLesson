...
/**
 * User: 蔡从稳 Date: 14-3-12 Time: 下午5:02
 * 
 * 热门搜索的功能 
 * 
 */
...
public class AutoWrapLinearLayout extends LinearLayout {
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		int cellWidthSpec = MeasureSpec.makeMeasureSpec(0,
				MeasureSpec.UNSPECIFIED);// 让自控件自己按需撑开

		int cellHeightSpec = MeasureSpec.makeMeasureSpec(0,
				MeasureSpec.UNSPECIFIED);// 让自控件自己按需撑开
	...
	private void foo() {
		...
		mHeight = paddingTop + y + cellMarginTop + cellHeight
				+ cellMarginBottom + paddingBottom;
	}
}